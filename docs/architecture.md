...
**最終的な解決策**：3の`nuxt-og-image` + `deployConfig: true`を採用し、3で衝突していた`env.preview`ブロックを`wrangler.jsonc`から削除、プレビュー環境専用の設定を独立した`wrangler.preview.jsonc`ファイルに分離した（詳細は§11.1参照）。CIのプレビュービルド時にこのファイルを`wrangler.jsonc`へコピーしてからビルドすることで、`deployConfig`とマルチ環境構成を両立させている。`wrangler dev`/`wrangler deploy`実行時に日本語フォント込みでOGP画像が正しく生成されることをローカル環境で確認済み。
...
| OGP画像生成              | takumi-rsベースの`nuxt-og-image`モジュール（直接`@takumi-rs/wasm`を呼ばない）                                                                                                                                            | satori比で2〜10倍高速。ただし`@takumi-rs/wasm`単体の直接利用はCloudflare Workers本番ビルドで解決不能な問題があり（§10）、公式モジュール経由に変更                                                                      |
...
| OGP画像のURLパス         | 既存の `/ogp/{slug}.png` を踏襲せず、`nuxt-og-image`が自動発行するルート（`/_og/...`配下）を採用。独自の`/api/og/:slug`は実装しない                                                                                      | 旧パスへの追従・リダイレクト対応は行わない。過去のSNSシェアはプラットフォーム側にキャッシュされていることが多く実害は小さいと判断                                                                                      |
